...
if uploaded_file is not None and st.sidebar.button("🚀 Ingest & Index Document"):
    with st.spinner("Extracting structural layout data..."):
        try:
            ...
            if len(raw_text) == 0:
                st.error("❌ Extracted text layer is empty. Verify file formatting strings.")
                st.stop()
                
            # Manual text slicing parameters
            size = 2000
            overlap = 200
            text_chunks = []
            ...
            # Cache the arrays into Streamlit's session memory state

...
def crawl_site_pages(root_url, max_pages=3):
    ...
    # Save the total number of pages crawled into Streamlit's state memory
    st.session_state.pages_crawled_count = len(visited_urls)
...
if "vector_store" in st.session_state:
    ...
    if user_query:
        with st.spinner("Combing vector store database..."):
            # UPGRADE 1: Track the exact database search speed
            ...
            # UPGRADE 1 (Continued): Visual Metrics Scorecards
            col1, col2 = st.columns(2)
            with col1:
                st.metric(label="📄 Total Web Pages Crawled", value=st.session_state.get('pages_crawled_count', 'N/A'))
            with col2:
                st.metric(label="⏱️ Vector Search Speed", value=f"{search_duration} seconds")
            
            st.markdown("### 🤖 AI Response:")
            st.info(response["answer"])
            
            # UPGRADE 2: Data Source Expander (Collapsible drop-down showing raw chunks)
            with st.expander("🔍 View Raw Verified Data Sources (ChromaDB Chunks)"):
                st.write("The AI generated the answer above using these exact text snippets retrieved from the website:")
                for index, doc in enumerate(response["context"]):
